...
int main(int argc, char *argv[])
{
    ...
    // 进行语法分析并构建AST
    std::cerr << "Starting syntax analysis and AST construction..." << std::endl;
    ...
    if (!root)
    {
        std::cerr << "AST construction failed: root is null" << std::endl;
        if (yyin != stdin)
        {
            fclose(yyin);
        }
    ...
    std::cerr << "Syntax analysis completed successfully." << std::endl;
